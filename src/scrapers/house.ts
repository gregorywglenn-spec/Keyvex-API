...
/**
 * House PTR PDFs encode asset class via a `[XX]` bracket after the ticker
 * (e.g., "Apple Inc Common Stock (AAPL) [ST]"). The 2-char codes themselves
 * are not human-friendly — agents asking "show me bonds" want
 * asset_type="Government Security", not asset_type="GS". Map at parse
 * time so the stored value is what an agent would query for.
 *
 * Codes that fall through unmapped (rare but possible — House Clerk
 * occasionally adds new types) are stored as the raw 2-char code so
 * agents can still filter on the source-of-truth value.
 */
...
/**
 * Asset types that don't have a public stock ticker. The `[XX]` ticker
 * regex is greedy enough to grab "BART" from "San Francisco CA BART [GS]"
 * — that's the muni-bond name's terminus, not a stock symbol. Same for
 * corporate-bond CUSIP fragments and LP/LLC suffixes on real-property
 * and other rows. Force ticker=null on these so the bond doesn't survive
 * as a phantom equity in tickerwise queries.
 */
const NO_TICKER_TYPE_CODES = new Set(["GS", "CO", "RP", "OT"]);

/**
 * Conservative blacklist of "tickers" the parser might extract from
 * parens at the end of an asset name but that are NEVER valid US stock
 * symbols. Catches:
 *   "Multi-Pack Solutions, LLC [OP]"  → would set ticker="LLC"
 *   "Apollo Strategic Growth LP [ST]" → would set ticker="LP"
 *   "Trust Series III [ST]"           → would set ticker="III"
 *
 * Critically does NOT include 2-letter state abbreviations (MA = Mastercard,
 * DE = Deere, MO = Altria, IT = Gartner are real tickers), and excludes
 * Roman V (= Visa) and X (= US Steel). I, II, III, IV, VI–IX are safe —
 * none collide with current US-listed tickers.
 */
const TICKER_BLACKLIST = new Set([
  // Corporate entity suffixes
  "LLC", "LP", "GP", "INC", "CORP", "LTD", "PLC", "NV", "BV",
  // Roman numerals — keep only those that don't collide with real tickers
  "I", "II", "III", "IV", "VI", "VII", "VIII", "IX",
  // Currency codes (occasionally appear as last word of a fund name)
  "USD", "EUR", "GBP", "CHF", "JPY", "CAD", "AUD", "CNY", "HKD",
]);
...
export function parseHousePtrText(
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // ─── Owner detection (hybrid regex) ──────────────────────────────
    //
    // Two source-data realities to defend against:
    //   (a) Concatenated "SPApple" / "JT3M" / "DCBJ" — pdf-parse strips
    //       the visual whitespace between the owner code and the next
    //       glyph, so the marker fuses with the asset name. Our pattern
    //       has historically iterated through 4 versions to handle all
    //       observed punctuation/case mixes (SPiShares, JTT., JTO',
    //       DCBJ, etc.) — see CLAUDE.md "House owner-code regex" lesson.
    //   (b) Standalone "SP Apple" / "JT 3M" — when pdf-parse occasionally
    //       preserves the space, OR when the source PDF was generated by
    //       a different upstream tool. Derek's parser uses pdfjs-dist
    //       which preserves whitespace more reliably.
    //
    // Both patterns must work because the same scraper run can encounter
    // both formats across different filings. SPDR is the known false-
    // positive for the concatenated pattern (SPDR S&P 500 ETF starts
    // with "SP" + non-space + content) — the SPDR allowlist below
    // rejects it explicitly while keeping every legitimate concatenated
    // SP/JT/DC marker.
    let owner: "Self" | "Spouse" | "Joint" | "Dependent" = "Self";
    let withoutOwner = fullAsset;

    // Pattern 1: standalone token "SP Apple" / "JT 3M" — token surrounded
    // by whitespace boundaries
    const standaloneOwner = fullAsset.match(/^(SP|JT|DC)\s+(\S.*)$/);
    if (standaloneOwner) {
      owner = normalizeOwner(standaloneOwner[1]!);
      withoutOwner = standaloneOwner[2]!.trim();
    } else {
      // Pattern 2: concatenated "SPApple" / "JT3M" — owner code fused to
      // start of asset name. False-positive guard: SPDR ETFs begin with
      // "SP" + "DR" + content. Reject those (asset stays Self-owned).
      const concatOwner = fullAsset.match(/^(SP|JT|DC)(\S.*)$/);
      if (concatOwner) {
        const code = concatOwner[1]!;
        const rest = concatOwner[2]!;
        const isSpdrFalsePositive = code === "SP" && /^DR\b/i.test(rest);
        if (!isSpdrFalsePositive) {
          owner = normalizeOwner(code);
          withoutOwner = rest.trim();
        }
      }
    }
    ...
    // Apply ticker blacklist — reject LLC/LP/INC/Roman numerals/currency
    // codes that the regex above would otherwise capture as fake tickers.
    if (ticker && TICKER_BLACKLIST.has(ticker.toUpperCase())) {
      ticker = "";
    }

    // Force ticker=null for non-equity asset types (Government Security,
    // Corporate Bond, Real Property, Other) where the [XX] regex may have
    // captured a bond name's terminus or LP/LLC fragment as a fake ticker.
    if (ticker && NO_TICKER_TYPE_CODES.has(assetTypeCode)) {
      ticker = "";
    }

    // Translate the 2-char asset type code to a human-readable label so
    // agents can filter on natural names. Codes that fall through unmapped
    // (rare — House Clerk occasionally adds new types) preserve the raw
    // code so agents can still query the source-of-truth value.
    const assetType = ASSET_TYPE_MAP[assetTypeCode] || assetTypeCode;
    ...
    // Doc ID: stable across parser revisions. (docId, txDate-no-slashes,
    // rowIdx) means a same-day same-PTR row keeps its ID even if a future
    // parser fix yields one fewer trade earlier in the same PTR — only
    // rows that share BOTH the docId AND the txDate could collide on
    // rowIdx, and that's unusual (most filings have at most a handful of
    // trades per date). Replaces the previous fragile pattern
    // `house-${docId}-${trades.length + 1}` which shifted every doc ID
    // downstream of any parser change.
    const txDateCompact = txDate.replace(/\//g, "");
    trades.push({
      id: `house-${meta.doc_id}-${txDateCompact}-${trades.length}`,
