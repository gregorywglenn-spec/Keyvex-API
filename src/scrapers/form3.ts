...
export async function scrapeForm3LiveFeed(
  ...
  if (hasStart !== hasEnd) {
    throw new Error(
      "Form 3 date-range mode requires BOTH startDate and endDate (got only one)",
    );
  }
  ...
  while (true) {
    ...
    console.error(
      `[form3 live]   page from=${from} returned ${pageHits.length} hits (total=${totalAvailable})`,
    );
    ...
    if (allHits.length >= PAGINATION_CAP) {
      console.error(
        `[form3 live] hit pagination cap ${PAGINATION_CAP}, stopping`,
      );
      break;
    }
  ...
  console.error(
    `[form3 live] ${allHits.length} Form 3 filings paginated (${modeDescription}, total available=${totalAvailable})`,
  );
