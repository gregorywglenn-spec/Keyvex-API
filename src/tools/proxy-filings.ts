...
export const definition: Tool = {
  ...
  description: [
    ...
    "",
    "Coverage: the full DEF 14A family back to 2016 for the US public-company",
    "universe (sourced from EDGAR's complete quarterly full-index); a daily",
    "feed keeps it current. Rows are tagged with a company's PRIMARY common",
    "ticker — for dual-class issuers (e.g. GOOGL/GOOG, BRK-A/BRK-B) query by",
    "company_cik to retrieve every share class in one shot.",
    "",
    ...
    "  - Recent-window rows (filed ~2024-onward via the daily feed /",
    "    per-ticker pull): DEF 14A primaries ~100% populated (meeting/record",
    "    date); DEFA14A/DEFM14A/DEFR14A typically EMPTY (correct-as-filed —",
    "    SEC's submissions API leaves those reportDate fields blank).",
    "  - Historical backfilled rows (the bulk of 2016-2024 depth, sourced",
    "    from EDGAR's full-index): period_of_report is EMPTY for all form",
    "    types — the index carries no report date.",
    "  - Bottom line: filter/sort by filing_date for chronological queries;",
    "    period_of_report is not reliably present across the collection.",
    "",
