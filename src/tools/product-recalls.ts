...
export const definition: Tool = {
  ...
  description: [
    ...
    "",
    "Source freshness (per-source publication cadence, not KeyVex bug):",
    "  CPSC publishes within ~1-2 days; recent data flows hourly-fresh.",
    "  openFDA's snapshot updates every ~10-14 days, and each snapshot",
    "  carries recall_initiation_date values that LAG the snapshot date",
    "  by another 30-45 days (the time between FDA classifying a recall",
    "  and openFDA exposing it). Net: FDA records in this collection",
    "  typically run ~4-6 weeks behind real-world recall dates, while",
    "  CPSC is current. A default desc-by-date sort therefore looks",
    "  CPSC-heavy at the top even when FDA matters more for the query.",
    "  Filter by source='fda_*' to see FDA-only and avoid the skew.",
    "",
    "classification filter scope: 'classification' is an FDA-only field.",
    "CPSC records always have classification=null (CPSC doesn't use the",
    "FDA severity scheme). Filtering by classification excludes ALL CPSC",
    "rows by definition. The query response surfaces this with a notice",
    "in coverage_warning when the filter is set.",
  ].join(" "),
  inputSchema: {
    type: "object",
    properties: {
...
export async function handler(
  ...
  const { results, has_more, coverage_warning } = await queryProductRecalls(query);
  return {
    results,
