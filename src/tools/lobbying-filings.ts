...
export const definition: Tool = {
  ...
  description: [
    ...
    "",
    "Income vs expenses (IMPORTANT for ranking by spend): the LDA",
    "mandates a hard split. Third-party lobbying firms report `income`",
    "(what the client paid them); in-house corporate lobbying departments",
    "report `expenses` (what they spent). The two fields are mutually",
    "exclusive — any given filing has one or the other, not both. In",
    "practice ~30% of filings have income, ~70% have expenses, with a",
    "small population reporting neither (administrative registrations).",
    "So `sort_by=income` ranks the third-party-firm subset; for an",
    "actual top-spenders leaderboard, agents should fetch both populations",
    "and sum `income + expenses` per filing client-side. v1.1 polish",
    "will add a derived `total_lobbying_spend` field that does this sum",
    "server-side for indexed queries. client_is_government is true when",
...
export async function handler(
  ...
  const { results, has_more, coverage_warning } = await queryLobbyingFilings(query);
  return {
    results,
