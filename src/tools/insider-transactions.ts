...
export const definition: Tool = {
  ...
  description: [
    ...
    "ticker or company_cik to be set. Baseline rows with is_nil_filing=true",
    "are 'no securities owned' Form 3s (~half of all filings) — the insider",
    "filed but started with ZERO holdings; shares_owned 0 is the position,",
    "not missing data.",
    "",
    ...
    "  intended year.",
    "",
    "  (3) NUMERIC PRECISION — for data_source='bulk_v2', shares and",
    "  price_per_share mirror SEC's BULK Form 345 extract, which rounds to",
    "  2 decimals (e.g. 474.6, where the primary XML shows 474.598). That",
    "  rounding is SEC's, in the bulk feed — KeyVex stores the bulk value",
    "  verbatim (no rounding in the loader). Audit v2 numerics against the",
    "  bulk extract (the source of record), not the XML primary document,",
    "  which carries fuller precision. Dates and transaction codes DO match",
    "  the XML exactly.",
    "",
    ...
    "meant. A customer auditing KeyVex against EDGAR's source of record for",
    "each row (the bulk Form 345 extract for v2 rows) will find a",
    "byte-for-byte match.",
    "",
