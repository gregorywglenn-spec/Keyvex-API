...
export const definition: Tool = {
  ...
  description: [
    ...
    "value at settlement.",
    "",
    "Note: FTDs are bi-monthly batch-published, not real-time. SEC releases",
    "each half-month batch (cnsfails<YYYYMM>a = days 1-15, b = 16-end) roughly",
    "2-4 weeks AFTER that half-month period closes, so the most recent",
    "settlement date can be 2-4 weeks behind today (e.g. in mid-June the latest",
    "published batch is first-half-May, with settlement dates through ~May 15).",
    "That apparent lag is the SEC publish cadence, not a KeyVex freshness gap.",
  ].join(" "),
  inputSchema: {
    type: "object",
    properties: {
