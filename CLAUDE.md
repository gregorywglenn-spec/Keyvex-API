...
- **Firestore range filters must align with the orderBy field — `where(sortField, ">=", date)` only works when sortField IS a date field.** Day 9: 9 query handlers had pattern `if (query.since) q = q.where(sortField, ">=", query.since)`. When `sort_by:"shares_owned"` and `since:"2026-01-06"`, this generated `where("shares_owned", ">=", "2026-01-06")` — comparing share counts to a date string, matching zero records. Bug was invisible because it returned 0 hits (genuine empty) not an error. Caught when `get_activist_stakes` queries with broad date filters returned 0/3 across 3056 records. **Lesson: any query handler that mixes server-side range filtering with a parameterized sort field MUST validate sortField is the canonical date field (or numeric range field) before applying. Otherwise defer to client-side filtering. Applies to: insider_trades, congressional_trades, planned_insider_sales, initial_ownership_baselines, activist_ownership, federal_contracts, lobbying_filings, material_events, annual_financial_disclosures.**
- **Firestore disallows range filters on multiple fields without a multi-field index.** Day 9: `where(min_value).where(since/until).orderBy(date)` triggers "The query contains range and inequality filters on multiple fields" — even when each individual filter has its own index. Caught when `get_insider_transactions(SCHW, min_value:5M, sort_by:"transaction_date")` failed. **Lesson: server-side `min_xxx` filter only applies when sortField IS that value field (so inequality + orderBy align on same field). Otherwise defer min_xxx client-side. Same rule applies to all handlers with `min_value`, `min_amount`, `min_income`, `min_percent_of_class` params.**
- **Firestore composite indexes with DESC sort don't auto-mirror to ASC queries.** Day 9: had `(ticker, total_value DESC)` index for "biggest first" queries; missed `(ticker, total_value ASC)` for "smallest first" or "earliest first" sort_order:asc variants. Each (filter set, sort field, sort direction) combination needs its own composite index. Random sort_order in tests surfaces these gaps quickly — production users almost always sort DESC ("most recent" / "biggest first") so ASC variants get added on-demand. v0.18.0 of insider_trades has 4 ASC variants for the most common combos.
- **fast-xml-parser default `maxNestedTags` is 100 — bump it for any deep SEC filing parser.** Day 9: WMT 2024 SC 13G/A filings stack subsidiary disclosures 100+ levels deep (each reporting-person block contains nested holding entities). Parser silently emits "Maximum nested tags exceeded" SKIP and zero records. Same root cause as the Form 144 v1.1 polish item from Day 3 night, now fixed for 13D/G parser. **Lesson: in the XMLParser config, set `maxNestedTags: 5000` (with `as ConstructorParameters<typeof XMLParser>[0]` cast since v4 typed signature omits it). Apply preemptively to every SEC scraper — Form 4, Form 3, Form 144, 13D/G, 13F, 8-K, Form 278.**
...
### Day 9 (2026-05-09) — DNS finalization + 5-round commercial-readiness sprint

Marathon session focused on shipping the MCP server to production-quality. Five sequential bug-find / fix / verify rounds, big coverage push on activist_ownership, and final DNS landing for the brand domain.

**1. GoDaddy DNS — `keyvex.com` apex + `www.keyvex.com` LIVE with TLS.**
- Started session with `mcp.keyvex.com` already mapped (Day 7), but apex and www were still parked at GoDaddy's aftermarket lander page.
- Walked through GoDaddy DNS UI step by step: edited A record from "Parked" → `199.36.158.100`, edited www CNAME from `keyvex.com` → `capitaledge-api.web.app`, added 3 new TXT records (hosting-site verification + 2 ACME challenges).
- Records propagated globally in ~15-20 min. Firebase polled DNS, ownership flipped ACTIVE, Let's Encrypt cert provisioned. Both `https://keyvex.com` and `https://www.keyvex.com` now serve the landing page with TLS.
- The Firebase Hosting REST API at `src/firebase-rest.ts` (built Day 7 per Derek's tip) was load-bearing throughout — bypassed Chrome MCP allowlist to inspect cert state, ACME tokens, ownership state.

**2. Five-round commercial-readiness sprint (battle test driven).**

Round 1 — `fix(round 3): server-side filter alignment with sortField across 9 query handlers` — commit `7bbba77`:
- Bug: 9 query handlers had `if (query.since) q = q.where(sortField, ">=", query.since)`. When `sort_by:"shares_owned"` and `since:"2026-01-06"`, generated `where("shares_owned", ">=", "2026-01-06")` — comparing share counts to a date string. Matched zero records, returned silent empty.
- Caught when randomized battle test #10 surfaced `get_activist_stakes 0/3` despite collection having 3,056 records.
- Fix: server-side date filter only when `sortField === dateField`; defer to client-side otherwise. Applied across insider_trades, congressional_trades, planned_insider_sales, initial_ownership_baselines, activist_ownership, federal_contracts, lobbying_filings, material_events, annual_financial_disclosures.
- Pre-fix: 77% data hit, 2 failures, 617ms avg → post-fix: 84% data hit, 0 failures, 1037ms avg.

Round 2 — included in same commit `7bbba77`:
- Bug: Firestore disallows range filters on multiple fields. `where(min_value)` (range on total_value) + `orderBy(disclosure_date)` triggered "range and inequality filters on multiple fields" error.
- Caught when `get_insider_transactions(SCHW, min_value:5M, sort_by:"transaction_date")` failed.
- Fix: server-side `min_xxx` only when `sortField === <value_field>`. Otherwise defer client-side. Applied to all 5 handlers with min_xxx params.

Round 3 — `fix(round 4): activist parser depth + insider_trades ASC indexes + realistic test` — commit `ea7ab3d`:
- Two improvements:
  1. fast-xml-parser `maxNestedTags` bumped 100 → 5000 (closes the v1.1 polish item from Day 3 about Form 144 — same root cause, applied to 13D/G now).
  2. New `--max-filings=N` CLI flag on `13d-13g-feed` — default 100 per form code was burying big-cap filings beyond the cap.
- Plus 4 new ASC indexes for insider_trades (`(ticker, total_value ASC)`, etc.) — caught by random sort_order in the enhanced battle test.
- Battle test enhanced from 30 → 52 queries with 5 new realistic templates (concentrated activist stakes, BlackRock/Vanguard 13G hunting, biggest insider buys, earnings 8-Ks, registrant-side lobbying) + composed-session simulation (3-step agent flows like the political-alpha LMT play).

Round 4 — `feat(round 5): targeted 13D/13G scraper by issuer CIK + big-cap backfill` — commit `2f2b9d0`:
- The big-cap activist coverage gap from earlier rounds turned out to be parser cap, not HTML format. Cross-issuer FTS feed paginated all 10K hits per form code per year but only PARSED the first 100. Vanguard/BlackRock annual 13G/A on big-caps (one filing per ticker per year) landed at hit position #500+, never got parsed. Filing format IS structured XML.
- New `scrapeActivistByIssuerCIKs(ciks, dateRange)` in `activist.ts` — queries EDGAR FTS with `&ciks=<cik>` filter for each (CIK × form code) pair. Each query returns ~5-20 hits per big-cap per year, well under the 100-hit page cap.
- New CLI command `13d-13g-issuers --tickers=A,B,C` or `--big-caps` with built-in S&P top-60 list (`BIG_CAP_TICKERS` constant in `scrape.ts`).
- Backfill run 2024-01-01 → 2026-12-31 with `--big-caps`: 1,387 new ownership rows, `activist_ownership` 3,056 → 4,443 records, big-cap coverage 0/15 → 20/29 verified tickers. Vanguard's 9.47% 2025-07-29 13G/A on AAPL ($1.4B shares) landed correctly. Plus April 2026 Vanguard 7.48% 13G ($1.1B shares).

**3. Battle test methodology — randomized-realistic.**
- Started day at 30 single queries × 22 templates. Ended at 52 queries (50 singles + 2-step composed session) × 27 templates.
- New "composed session" mode simulates a real agent's multi-step query chain (political-alpha LMT play, CEO insider tracker, lobbying→trades, activist watchlist).
- 8 total test runs after fixes (rounds 1-5). Average across all post-fix runs: 84% data hit, 0.14 failures, ~1100ms latency.
- Tool-level wins: `get_activist_stakes` went from 0/3 with data → 10/11 with data (was the worst-performing tool, became one of the best).

**Net session delta:**

| Metric | Session start | Session end |
|---|---|---|
| Battle test data hit | 77% | 85-88% |
| Battle test failures | 2 per run | 0 per run |
| Avg query latency | 617ms | ~984-1100ms (still under 1s for non-historical queries) |
| `activist_ownership` records | 3,056 | 4,443 (+45%) |
| Big-cap activist coverage | 0/15 | 20/29 |
| `get_activist_stakes` per-tool data hit | 0/3 (0%) | 10/11 (91%) |
| Brand domain TLS coverage | mcp.keyvex.com only | apex + www + mcp |

**Customer-visible outcome:** an agent asking "who has 5%+ of AAPL?" now returns Vanguard's 9.47% July 2025 13G/A. Same for MSFT, NVDA, META, AMZN, TSLA, WMT, LLY, XOM, and 12 other major names. The political-alpha cross-source pattern (`get_congressional_trades(LMT) → get_member_profile → get_federal_contracts → get_material_events`) returns substantive results across all four tools.

**Open v1.1 polish queue (from today):**
- Some big-caps (JPM, BAC, GS, BLK, GOOGL, BA) still show 0 records keyed to themselves. CIK queries returned filings filed BY them (correctly stored against OTHER tickers). JPM/GS/BAC are genuinely rare 13G subjects due to size + shareholder fragmentation. Acceptable.
- HTML-only 13G filings (some smaller filers ship HTML format, no structured XML) — would need a separate parse path. Not blocking; structured XML is the dominant format post-Feb-2024 mandate.
- Pre-2024 13D/G filings predate the structured-XML mandate (paper / HTML format). Parser silently emits 0 rows. Captured as Hard Lesson on Day 4. Acceptable; live feed is current-window focused.
- `legislators_historical` queries on member_name substring are slow (4179ms on "Sumner") because they scan 12,230 records. v1.1 fix would tokenize names server-side.
