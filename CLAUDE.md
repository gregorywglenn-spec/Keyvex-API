...
June 2, 2026 — **37 MCP tools at `mcp.keyvex.com` (v0.52.x live)**. This session shipped two things: **(1) FINRA OTC fully removed AND deployed** — function deleted, `mcp`+`scheduledHealthCheck` redeployed, 4 indexes dropped, `otc_market_weekly` collection deleted, pushed to origin/main (`90517ef` + `031742c`); live-verified 37 tools, tool gone. **(2) KeyVex Google billing consolidated** onto the KeyVex account/card (`claude1986aaa` / "KeyVex Billing" `0124D1-ECB30F-C0E2BF` / Visa ••4155) — both Firebase projects re-linked off Greg's personal Visa ••0847 (see the 2026-06-02 billing entry above + `project_billing_setup.md`). Prior session (May 15): read-only-SA hardening + final three scrapers (FARA, Form 5, CSL). Building phase closed — next focus is promotion and launch (MCP registry submissions, launch posts). Day 10 Track A + B reconciled into v0.43.0 (see Session Bootstrap).
