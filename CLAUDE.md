...
4. **"Done" means FOUR checkable facts: committed + pushed + MERGED-TO-MAIN + deployed-and-verified.** (Merge was added 2026-06-08 after the discovery that ~48 commits — 2 months of fixes — had deployed off feature branches but never landed on main; `origin/main` had silently fallen behind production the whole time. Deploy ships the working tree, so production worked while main rotted, with no symptom until someone diffed them.) The correct flow is **commit → push → merge to main → deploy from main**, done as ONE closing motion at the end of a completed, verified unit of work — Greg should NOT have to say "merge" separately; treat it as the standard last step of "done" and announce it as you do it. EXCEPTION: if the merge isn't clean/safe (main diverged, conflicts, or it would pull in unreviewed/unexpected commits — as on 2026-06-08), STOP and surface it for Greg's decision instead of auto-merging. Never tell Greg something is "done" until all four hold — confirm with `git log origin/main` (it must contain the work) and a live `curl https://mcp.keyvex.com`. Narrative memory is unreliable across compaction and restarts; git and the live endpoint are not.
   - **Session-start hygiene check (do this in Session Bootstrap):** run `git rev-list --count origin/main..HEAD` and `git log origin/main..HEAD --oneline` to see whether the current branch is ahead of main (work not yet merged), and scan `git branch -r` for other `claude/*` branches ahead of main (stranded work from other sessions). If main is behind, that's a merge debt to surface — don't let it accumulate silently again.
...
- **Firestore composite-index changes go through `firestore.indexes.json` + `firebase deploy --only firestore:indexes` — NEVER the console "create index" URL.** The honest `INDEX_MISSING` errors KeyVex returns include a one-click Firebase console URL that creates the missing index. Clicking it provisions the index in *production* but NOT in the repo file → silent drift. Caught 2026-05-28 during the directory-submission audit: the file declared 201 composite indexes while production had **250 deployed** — 49 had drifted in via console URLs + the Day-10 parallel-worktree divergence. Drift is dangerous because `firebase deploy --only firestore:indexes` reconciles file→production and will prompt to **DELETE** any production index absent from the file — a careless "yes" drops live indexes and starts throwing `INDEX_MISSING` in prod. **Discipline:** when an `INDEX_MISSING` surfaces, add the index to `firestore.indexes.json` and deploy — do not click the URL. If drift is suspected, reconcile by capturing `firebase firestore:indexes` (read-only), stripping the implicit `__name__` field entry + the `density` key from each entry, mirroring that into the file, then re-adding any pending new indexes on top so the next deploy is purely additive (file == production + N). The 2026-05-28 reconciliation restored the file to a verbatim 250-index mirror + 2 new congressional_trades indexes = 252.
...
### 🚫 2026-06-01 — FINRA OTC dropped (SRO/licensed, not a US-gov source)

While diagnosing the two health-check "dead scraper" alerts, Greg flagged the real issue with the FINRA OTC weekly scraper: **FINRA is a self-regulatory organization (SRO), not a US-government source, and its data API requires a license agreement.** That conflicts with KeyVex's pure-public-US-government-source posture, so FINRA OTC was removed entirely (NOT fixed). This is a posture/legal decision, not a bug fix.

**What was removed (code committed + staged; deploy / collection-delete / push are separate Greg-go gates):**
- `src/scrapers/finra-otc.ts`, `src/tools/otc-market-weekly.ts`, `scripts/smoke-otc-tool.ts`, `scripts/count-otc.ts` (deleted)
- The `get_otc_market_weekly` MCP tool registration, the `unified_search` fan-out + identifier-coverage description, the `scrapeFinraOtcWeekly` Cloud Function + its two imports, the `otcMarketWeeklySync` health-check job, the `OtcMarketWeekly`/`OtcMarketWeeklyQuery` types, the `queryOtcMarketWeekly`/`saveOtcMarketWeekly` firestore layer + collection-default-sort entry, the `finra-otc` CLI command, and the 4 `otc_market_weekly` composite indexes.
- Scrubbed all customer-facing surfaces: landing (`marketing/site/index.html` — sources grid, demo example, prose, counts), `docs/index.html` tool table, the marquee `demo.svg` (otc row → `proxy_filings`), `terms.html` + `security.html` + `privacy.html` (FINRA had been listed as a claimed data source in legally-binding text — corrected), `README.md`, `registry-submissions.md`, and the obsolete `docs/handoff-monitoring-finra-fix-2026-05-31.md` (deleted). `HANDOFF_DEREK_SCRAPERS.md` scraper #6 marked **DROPPED — do not port** so Derek doesn't rebuild the licensed source.
- **Tool count 38 → 37.** Both `tsc` projects clean after removal.

**Gated items — ALL COMPLETED 2026-06-02 (Greg gave the go):** (1) deployed — `scrapeFinraOtcWeekly` deleted via `functions:delete`, `mcp` + `scheduledHealthCheck` redeployed, the 4 `otc_market_weekly` indexes removed (diff-gated to exactly those 4, no drift); (2) `otc_market_weekly` collection deleted (verified count = 0); (3) pushed to origin/main (`90517ef` + README count-reconciliation `031742c`). Live-verified on the wire: `mcp.keyvex.com` → v0.52.x, 37 tools, `get_otc_market_weekly` gone; battle test 0-error. (The deployed function had been crash-looping weekly on `Unexpected end of JSON input` — a "done≠deployed" gap where the record-total=0 guard from `f885374` was committed but never deployed; moot now it's deleted.)

**HHS-OIG was NOT dropped — it's a genuine US-gov source (HHS Office of Inspector General) and stays.** It wasn't broken: the CSV endpoint + parser are healthy; its first scheduled fire (`0 7 5 * *`, the 5th of the month) simply hadn't come around since the function was created May 12. It self-clears on its next run (June 5) or via a manual seed run.

### 💳 2026-06-02 — KeyVex Google billing consolidated onto the KeyVex account (off Greg's personal card)

Long Console session with Greg untangling *who pays for KeyVex on Google*. This is load-bearing infra — recorded here and in memory (`project_billing_setup.md`).

- **The problem (a text alert about a personal charge surfaced it):** BOTH Firebase/Cloud projects — `capitaledge-api` AND `capital-edge` (`capital-edge-d5038`) — were billing to a **personal** Cloud Billing account ("My Billing Account", `011EC6-C9E46C-C3F4F5`) paid by Greg's **personal Visa ••0847**, under his personal Google account `gregorywglenn@gmail.com`. Greg owns *both* Google accounts (`claude1986aaa` = KeyVex business, `gregorywglenn` = personal).
- **Two-name trap (important):** there were 3 billing accounts and **two were named identically "My Billing Account"** (personal `...C3F4F5`/Visa ••0847, and the KeyVex one `...C0E2BF`/Visa ••4155). The project **Change-billing dropdown filters by NAME only, not ID** — so we **renamed** the KeyVex account `0124D1-ECB30F-C0E2BF` → **"KeyVex Billing"** to disambiguate before re-linking. (A 3rd account, "Firebase Payment" `015233-4087B6-D03923`, also on personal Visa ••0847, is now empty/0-projects — leftover Firebase default; **left open**, not closed.)
- **The fix:** re-linked BOTH projects from `...C3F4F5` → **"KeyVex Billing"** (`0124D1-ECB30F-C0E2BF`, KeyVex business **Visa ••4155** exp 05/30, under `claude1986aaa`). Verified on-screen: both projects show "KeyVex Billing"; that account's sole Billing Account Administrator = `claude1986aaa`.
- **Posture locked:** ALL KeyVex Google billing → "KeyVex Billing" / Visa ••4155 / `claude1986aaa`. **Never bill KeyVex to the personal account/card.** Personal wallets (`...C3F4F5`, "Firebase Payment") **stay OPEN** — they pay Greg's *other personal* stuff; do NOT close them.
- **Concept locked (Greg asked):** Firebase is **NOT** a separate bill from Cloud — a Firebase project *is* a GCP project; Blaze usage bills through the linked Cloud Billing account (one bill per project, itemized by service).
- **Boundaries observed:** Claude never typed a card number and never performed any IAM/billing change — Greg did every click/credential (read-tier browser); Claude guided + read the screen via screenshots. Same rule applies to all future billing work.
- **Left optional / NOT done (Greg's explicit call — "if it doesn't affect billing I'm fine"):** project-level IAM cleanup, i.e. whether `gregorywglenn` still holds Owner/Editor *on the projects themselves* (`capitaledge-api`/`capital-edge`). Separate system from billing; doesn't affect charges. If ever removed: ensure `claude1986aaa` is Owner FIRST (never remove the last Owner). Also a one-time pre-switch usage "tail" settles once on whatever account it was on before the 06-02 switch.

### 🏁 2026-06-10/11 — THE RECONCILIATION SWEEP COMPLETED (21 → 37 of 38 datasets)

A two-day marathon closed the data-completeness sweep that `docs/reconciliation/SWEEP-STATUS.md` tracks (that file remains the detailed record — start there). Highlights:

- **16 datasets reconciled** to verified coverage with reproducible adapters + clickable reports, including the big one: **insider_transactions_v2 verified 81/81 quarters count-exact vs SEC's own bulk TSVs (18.4M docs, 0 mismatches)**.
- **~12 production bugs found, fixed, AND deployed** — the recurring disease was *publication-lag-vs-window-field*: crons windowing on event dates for sources that publish late (Form 278 House index-year, FDA recall_initiation_date, CFPB receipt-lag) silently lose everything published after its event date ages out. Plus: hidden caps (GovInfo 500), dead legs hidden by per-source try/catch (DOJ since 5/15, OCC since 5/22), checkpoint-during-dry-run (13F Berkshire saved 0), newest-first processing letting originals overwrite amendments (13F), SEC catalog convention change (BRK.B → BRK-B hyphens broke dot-strip lookups; MMC renamed to MRSH).
- **DOJ rehomed OFF GCP**: justice.gov IP-blocks ALL Google Cloud egress (probe-verified). The pull now runs as a GitHub Actions daily cron (10:45 UTC) POSTing raw pages to the `dojIngest` Cloud Function, authenticated by GitHub's OIDC token (repo-claim verified, ZERO stored secrets). Verified end-to-end; health-monitored as its own job. **Pattern for any future gov-site-blocks-GCP case.**
- **Live-first passthroughs upgraded** (Greg's standing posture: live API answers, cron-fed cache as outage fallback): CFPB + federal contracts + grants now push all verifiable filters server-side and return authoritative `total_count` (CFPB 15.7M complaints; "wells fargo" → 168,909). Volume questions are finally honest.
- **Greg's three policy calls implemented**: FARA terminated registrants kept-as-history with status flags; Form 3 nil-marker rows (52.84% → 100% recent-window); CFPB stays passthrough (bulk-CSV is the subscriber-complaint contingency).
- **insider v2 continuation automated**: `scrapeForm345BulkQuarterly` (monthly-fire, no-op until SEC publishes the next quarter) + meta-driven recency boundary on `get_insider_transactions` — the tool's coverage warning advances itself each quarterly load.
- **One self-driving tail**: N-PORT holdings era residual (~3.3K filings) drains via the twice-daily healing cron by ~2026-06-14 — then re-verify coverage-by-day and drop the cron to one tick (note in SWEEP-STATUS).

**Last Updated**

June 13, 2026 — **Reconciliation sweep COMPLETE: 38 of 38 datasets verified against source.** The final two closed this session: insider_transactions_v2 (81/81 quarters count-exact vs SEC bulk TSVs, 18.4M docs, 0 mismatches; + recency-boundary warning + monthly quarterly-load cron) and N-PORT holdings (era 99.99%, backlog a true 0). The N-PORT tail surfaced and fixed a *class* of scale bug — unbounded `.get()` snapshots and accumulate-then-save loops that OOM at collection/record scale (now: stream the diff, stream the save, loop-not-spread). The one non-extracted N-PORT filing (PIMCO PCPI pre-launch seed N-PORT, holdings only in a 42MB HTML exhibit) is classified + marked terminal (`holdings_extraction_status`) so the backlog is genuinely 0; the HTML-exhibit parser is deliberately SHELVED (verified one-off; Quiver exposes no ETF N-PORT portfolio either). All fixes committed → pushed → merged to main → deployed → re-verified, per the audit standard. Tracked follow-ups live in SWEEP-STATUS — now PURELY polish, not coverage gaps (TOP one: a scale-safety audit of the other scrapers for the same OOM/unbounded-query class; plus Form 278 content-enrichment, class-share-hyphen sweep, 13F amendment-removal orphans, FEC Schedule E, 1-index drift, FARA fpIndex). Building + data-audit phases both closed; next focus is promotion/launch and the debugging/polish pass.
