...
**Gated items still pending Greg's explicit go (each):** (1) `firebase deploy --only functions,firestore:indexes` — un-deploys `scrapeFinraOtcWeekly`, refreshes the `mcp` tool list + health-check, and deletes the 4 orphaned indexes; (2) delete the `otc_market_weekly` Firestore collection (the licensed ~184K rows — destructive); (3) push to origin. The deployed `scrapeFinraOtcWeekly` had also been crashing weekly on `Unexpected end of JSON input` (a "done≠deployed" gap — the record-total=0 guard from commit `f885374` was committed but never deployed); moot now that it's being removed.
...
June 1, 2026 — v0.44.0, **37 MCP tools at `mcp.keyvex.com`** (FINRA OTC `get_otc_market_weekly` removed — SRO/licensed source, off the pure-public-gov-source posture; code committed + staged, deploy / collection-delete / push gated on Greg). Prior session (May 15): read-only-SA hardening of the MCP function, then the final three scrapers (FARA foreign agents, Form 5 annual insider filings, Consolidated Screening List). FARA and CSL are tools (`get_foreign_agents`, `get_screening_list`); Form 5 feeds the existing `get_insider_transactions`. Building phase is closed — next focus is promotion and launch (MCP registry submissions, launch posts). Day 10 Track A + Track B were reconciled into v0.43.0 (two parallel worktrees merged — see the Session Bootstrap section for the process fix).
